...
impl GraphemeCursor {
    ...
    fn is_boundary(&mut self, chunk: &str, chunk_start: usize) -> Result<bool, GraphemeIncomplete> {
        if self.state == GraphemeState::Break {
            return Ok(true)
        }
    ...
    /// given, then retry.
    fn next_boundary(&mut self, chunk: &str, chunk_start: usize) -> Result<Option<usize>, GraphemeIncomplete> {
    ...
    /// given, then retry.
    fn prev_boundary(&mut self, chunk: &str, chunk_start: usize) -> Result<Option<usize>, GraphemeIncomplete> {
        if self.offset == 0 {
            return Ok(None);
        }
        if self.offset == chunk_start {
...
// (imdaveho) NOTE: Moved to the top.
// #![allow(missing_docs, non_upper_case_globals, non_snake_case)]
